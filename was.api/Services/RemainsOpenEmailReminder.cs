...
{
    public class RemainsOpenEmailReminder: BackgroundService
    {
        private readonly IServiceProvider _serviceProvider;
        private ILogger<RemainsOpenEmailReminder> _logger;
        public RemainsOpenEmailReminder(IServiceProvider serviceProvider, ILogger<RemainsOpenEmailReminder> logger)
        {
        ...
        {
            _logger.LogInformation("AM Reminder Email Scheduler started.");
                ...
                try
                {
                    ...
                    var query = _db.WPEmailReminderResult.FromSqlRaw(sql,
                                ...
                                .Select(f => new
                                {
                                    ...
                                    FormTitle = f.Title,
                                    f.SubmittedDate,
                                    AreaManger= new KeyVal {
                                        key=f.Zone,
                                        Value = _db.Users
                                         .Where(u => u.Zone == f.Zone)
                    ...
                    for (int i = 0; i < results.Count; i += batchSize)
                    {
                        ...
                        var batch = results.Skip(i).Take(batchSize)
                                .Select(async result =>
                                {
                                    ...
                                    var am = result.AreaManger.Value.Split("|");
                                    var toEmail = am[1];
                                    var ccEmail = new List<string> { SubmittedByParts[1] };
                                    ...
                                    var templateName = "EHS_to_AM_Reminder";

                                    await FireEmailReminder(emailService, innerDb, result.Id, toEmail,ccEmail, subject, templateName, placeholders);
                                });
        ...
        private async Task FireEmailReminder(IEmailService emailService, AppDbContext _db, long submissionId, string to, List<string> cc, string subject, string template, Dictionary<string, string> placeholders)
        {
            try
            {
                await emailService.SendTemplatedEmailAsync(to, subject, template, placeholders, cc);

                await _db.FormSubmissions
                    .Where(s => s.Id == submissionId)
                    .ExecuteUpdateAsync(s => s.SetProperty(p => p.ReminderSent, 2));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Failed to send AM reminder email to:{to}, subject:{subject}, placeholders:{placeholders.ToJsonString()}");
            }
        }
    }
}
